...
struct plugin_b_mixin {
    void dump(std::vector<std::string>& vec) const {
        vec.push_back("plugin-b");
    }
};

DYNAMIX_DEFINE_MIXIN(tap::dynamix_domain, plugin_b_mixin)
    .implements<tap::dump_msg>();

extern "C" {
SYMBOL_EXPORT void modify_object(tap::object& obj) {
    mutate(obj, dynamix::add<plugin_b_mixin>());
}
SYMBOL_EXPORT void release_object(tap::object& obj) {
    mutate(obj, dynamix::remove<plugin_b_mixin>());
}
}
