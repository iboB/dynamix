...
TEST_CASE("type class") {
    ...
    type_class tc = {};
    CHECK_THROWS_WITH_AS(dom.register_type_class(tc), "tc: register type class with empty name", dynamix::domain_error);

    tc.name = dnmx_make_sv_lit("custom");
    CHECK_THROWS_WITH_AS(dom.register_type_class(tc), "tc: register type class 'custom' with match function = null", dynamix::domain_error);
    ...
    CHECK_THROWS_WITH_AS(dom.register_type_class(tc), "tc: register type class with duplicate name 'custom'", dynamix::domain_error);
}
